...
export default {
  ...
  methods: {
    async findKH() {
      ...
      const user = localStorage.admin;
      const result = await axios.get(`http://localhost:3000/admin/find-adm-mail/${user}`, {
        headers: {
    ...
    async updateKH() {
      ...
      if (confirm(message) == true) {
        const token = localStorage.token;
        const result = await axios.put(`http://localhost:3000/admin/update-adm/${this.ID}`,
          {
            TEN: this.TEN,
    ...
    async changePass() {
      if (!this.PASS_NEW || !this.PASS_OLD) {
        ...
        if (confirm(message) == true) {
          const token = localStorage.token;
          const result = await axios.put(`http://localhost:3000/admin/update-pass-admin/${this.ID}`,
            {
